 ...
 *
 * The second argument in `page.addInitScript` is an object that contains the values you want to pass
 * into the injected function.
 * When the function runs in the page context, it receives this object as its argument.
 * This allows the injected script to access the `objectName`, `method`, and `exposedFunctionName` variables.
 */
export async function spyOn(page, objectName, method) {
  const messages = []
  ...
  await page.exposeFunction(exposedFunctionName, (...args) =>
    messages.push(args),
  )
  ...
  await page.addInitScript(
    ({ objectName, method, exposedFunctionName }) => {
      ...
      object[method] = (...args) => {
        ...
        window[exposedFunctionName](...args)
      }
    },
    // The second argument is an object that contains the values you want to pass
    // into the injected function.
    // When the function runs in the page context, it receives this object as its argument.
    { objectName, method, exposedFunctionName },
  )
 ...
 * @param {Function} [implementation] - Optional custom implementation for the stubbed method.
 * @returns {Promise<Array<any>>} - A promise that resolves to an array containing the captured calls and their arguments.
 */
export async function stubMethod(page, objectName, method, implementation) {
  const calls = []
  ...
  await page.addInitScript(
    /**
     * @param {{ objectName: string; method: string; exposedFunctionName: string; hasCustomImpl: boolean }} params
     */
    ({ objectName, method, exposedFunctionName, hasCustomImpl }) => {
      // Access the global object in the page context
      const object = window[objectName]
      ...
      object[method] = (...args) => {
        ...
        if (hasCustomImpl) {
          ...
          return window[`customImpl_${exposedFunctionName}`](...args)
        }
        // Default behavior: Do nothing or return undefined
      }
    },
    // Pass parameters to the injected script
    {
      objectName,
      method,
      exposedFunctionName,
